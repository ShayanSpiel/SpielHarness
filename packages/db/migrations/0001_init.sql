-- SpielOS: consolidated, migratable Postgres schema.
-- Plain Postgres, no Supabase-specific types or functions.
-- Apply on any Postgres 14+ to get the full harness runtime.
...
-- ── Files (the single source of truth for everything) ──────────
...
-- Versioned snapshot on every body/metadata change.
...
-- Backfill: keep updated_at current.
...
-- Explicit, queryable graph of references between files.
...
-- Trigger-driven relation refresh. Keeps file_relations consistent with metadata.
...
as $$
  -- Mark all role/workflow files as dirty so the after-update trigger recomputes.
...
-- ── Workspace variables (and secret refs) ──────────────────────
...
-- ── Seed a demo org for local development ─────────────────────
